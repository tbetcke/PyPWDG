...
def compute_facedata(nodes, faces, nonfacevertex, dim):
    ...
    # vertices is a 3-tensor.  For each (double sided) face, it contains a matrix of dim+1 coordinates.  The first dim of these
    # are on the face, the final one is the non-face vertex for the corresponding element
    vertices = numpy.array([[nodes[fv] for fv in fvs] + [nodes[otherv]] for fvs, otherv in zip(faces, nonfacevertex)])
    # M picks out the first coord and the differences to the others
    M = numpy.bmat([[numpy.mat([[1]]), numpy.zeros((1,dim))], [numpy.ones((dim,1))*-1, numpy.eye(dim)]])
    # Apply the differencing matrix to each set of coordinates
    dirs = numpy.tensordot(vertices, M, ([1],[1]))
    # Ensure that the directions live in the last dimension
    directions = numpy.transpose(dirs, (0,2,1))
    
    normals=numpy.zeros((len(vertices),dim))
    ...
    normals *= (-numpy.sign(numpy.sum(normals * directions[:,-1,:], axis = 1)) / dets ).reshape((-1,1))
    if dim==1: dets=numpy.ones(dets.shape,dtype=numpy.float64)
...
class SimplicialMeshInfo(object):
    def __init__(self, nodes, elements, elemIdentity, dim):
        self.dim = dim
        self.elements = elements
        self.nodes = np.array(nodes)
        self.elemIdentity=elemIdentity
        ...
        nev = self.dim+1        
        # The vertices associated with each face
        self.faces = numpy.array([tuple(e[0:i]+e[i+1:nev]) for e in self.elements for i in range(0,nev)])
        # The "opposite" vertex for each face    
        nonfacevertex = numpy.array([e[i] for e in self.elements for i in range(0,nev)])    

        self.nfaces = len(self.faces)
        self.ftoe = np.repeat(np.arange(self.nelts), self.dim+1)
...
class Topology(object):
    def __init__(self, meshinfo, boundaries):
        self.boundaries = boundaries
        # the face to vertex map        
        ftov = ss.csr_matrix((numpy.ones(self.nfaces * meshinfo.dim), meshinfo.faces.ravel(), np.arange(0, self.nfaces+1)*meshinfo.dim), dtype=int)
